...
client.on('group-participants-update', async (anu) => {
	...
	client.on('message-new', async (mek) => {
          ...
          case 'ping':
            ...
            break
				...
				case 'tts':
					...
					if (args.length < 2) return client.sendMessage(from, 'Qual txt vc quer q eu fale?', text, {quoted: mek})
					dtt = body.slice(9)
					ranm = getRandom('.mp3')
					rano = getRandom('.ogg')
					dtt.length > 300
       ...
       client.sendMessage(from, 'Este é o número do meu criador (versão simples), só manda msg se for urgente 🥱',MessageType.text, { quoted: mek} )
           break    
           case 'setname':
                if (!isGroup) return reply(mess.only.group)
			    if (!isGroupAdmins) return reply(mess.only.admin)
           ...
           case 'demote':
				...
				case 'promote':
					...
					if (mentioned.length > 1) {
						...
						mentions(`@${mentioned[0].split('@')[0]} ganhou o cargo de ADM.`, mentioned, true)
						client.groupMakeAdmin(from, mentioned)
					}
					break	
			     	case 'kick':
					...
					if (mentioned.length > 1) {
						...
						mentions(`Removi mermo... @${mentioned[0].split('@')[0]} tchau 🏃`, mentioned, true)
						client.groupRemove(from, mentioned)
					}
					break
				case 'listadmin':
